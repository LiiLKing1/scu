...
const Debates = () => {
  ...
  useEffect(() => {
    setIsLoading(true)
    setListError('')
    const q = query(collection(db, 'debatesRecordings'), orderBy('createdAt', 'desc'))
    let arr = []
  ...
  const handleDelete = async (idx) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'debatesRecordings', rec.id))
    } catch (e) {
  ...
  const saveEdit = async () => {
    ...
    try {
      setEditError('')
      setEditSaving(true)
      await updateDoc(doc(db, 'debatesRecordings', rec.id), {
        title: editName,
        desc: editDesc,
      })
  ...
  return (
    <section className="select-none">
      <h2 className="text-[#0e1728] text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-center">Debates</h2>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    await addDoc(collection(db, 'debatesRecordings'), payload)
